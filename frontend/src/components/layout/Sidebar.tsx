...
export default function Sidebar({ items, role = 'student', userName, userEmail }: SidebarProps) {
  const [collapsed, setCollapsed] = useState(false)
  const navigate = useNavigate()
  const { user, logout } = useAuth()

  const displayName = user?.name || userName || 'User'
  const displayEmail = user?.email || userEmail || 'user@learnflow.co'
  ...
  return (
    <aside className={`flex flex-col bg-slate-900 text-slate-300 transition-all duration-200 ${collapsed ? 'w-16' : 'w-60'} flex-shrink-0 h-screen sticky top-0 z-30`}>
      {/* Logo */}
      <div className={`flex items-center gap-2.5 px-4 h-16 border-b border-slate-800 flex-shrink-0 ${collapsed ? 'justify-center' : ''}`}>
        ...
        {!collapsed && (
          <div>
            ...
            <span className="ml-1.5 text-xs bg-blue-600/20 text-blue-400 px-1.5 py-0.5 rounded font-medium">{roleColors[role] || role}</span>
          </div>
        )}
      </div>
        ...
        <ul className="space-y-0.5 px-2">
          {items.map((item) => (
            <li key={item.to}>
              <NavLink
                to={item.to}
                end={item.to === `/${role}` || item.to === '/admin' || item.to === '/lms/admin'}
                className={({ isActive }) =>
